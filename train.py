...
def train_phase(train_dataloader, optimizer, epoch,l1,l2,action_criterions):
    ...
    for data in train_dataloader:
        ...
        with torch.no_grad():
            ...
            #print(clip_feats.shape, " ssss sss ss s ss s ")
...
def main():
   ...
   # optimizer = optim.Adadelta(parameters_to_optimize,lr=0.0001,weight_decay=0.5)
    ...
    for epoch in range(num_epochs):
        saving_dir = "experimental_models"
        ...
        if epoch%4==0 or epoch==num_epochs-1 :
            save_model(model_CNN, 'model_CNN_relu', saving_dir,epoch)
            save_model(model_avg_fc, 'model_avg_fc_relu', saving_dir,epoch)
            save_model(model_reg, 'model_reg_relu', saving_dir,epoch)
            save_model(model_class, 'model_class_relu', saving_dir,epoch)
    grph.draw_and_save()
...
if __name__ == '__main__':
   ...
   # set_start_time1(str(datetime.today().strftime('%Y-%m-%d-%H:%M:%S')))
    ...
    if is_pretrained==False:
        ...
        model_CNN_pretrained_dict = torch.load('saved_models/model_CNN95.pth')
        model_avg_FC_pretrained_dict = torch.load('saved_models/model_avg_fc95.pth')
        model_class_pretrained_dict = torch.load('saved_models/model_class10.pth')
        model_reg_pretrained_dict = torch.load('saved_models/model_reg45.pth')
